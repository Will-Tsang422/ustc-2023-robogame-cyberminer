...
int main()
{
	...
	Size board_size = Size(6, 10);  //方格标定板内角点数目（行，列）
	...
	for (int i = 0; i < imgs.size(); i++)
	{
		...
		Mat img1_draw;
		img1.copyTo(img1_draw);
		drawChessboardCorners(img1_draw, board_size, img1_points, found);  //绘制方格标定板角点
		imgsPoints.push_back(img1_points);
	}

	//生成棋盘格每个内角点的空间三维坐标
	Size squareSize = Size(26, 26);  //棋盘格每个方格的真实尺寸
